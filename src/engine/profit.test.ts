...
describe('calculateNetProfit', () => {
  it('calculates positive profit correctly', () => {
    const vcredIn = 1000n * 10n ** 6n;  // 1000 VCRED (6 decimals)
    const vcredOut = 1050n * 10n ** 6n; // 1050 VCRED
    const fees = 10n * 10n ** 6n;       // 10 VCRED

    const profit = calculateNetProfit(vcredIn, vcredOut, fees);

    expect(profit).toBe(40n * 10n ** 6n); // 1050 - 1000 - 10 = 40
  });

  it('calculates negative profit (loss) correctly', () => {
    const vcredIn = 1000n * 10n ** 6n;
    const vcredOut = 980n * 10n ** 6n;
    const fees = 10n * 10n ** 6n;

    const profit = calculateNetProfit(vcredIn, vcredOut, fees);

    expect(profit).toBe(-30n * 10n ** 6n); // 980 - 1000 - 10 = -30
  });

  it('handles zero fees', () => {
    const vcredIn = 1000n * 10n ** 6n;
    const vcredOut = 1100n * 10n ** 6n;
    const fees = 0n;

    const profit = calculateNetProfit(vcredIn, vcredOut, fees);

    expect(profit).toBe(100n * 10n ** 6n);
  });

  it('handles break-even scenario', () => {
    const vcredIn = 1000n * 10n ** 6n;
    const vcredOut = 1010n * 10n ** 6n;
    const fees = 10n * 10n ** 6n;
...
describe('convertFeeToVcred', () => {
  it('converts ETH fee to VCRED correctly', () => {
    ...
    const ethToVcredRate = 2000n * 10n ** 6n; // 1 ETH = 2000 VCRED (rate in VCRED decimals)

    const feeVcred = convertFeeToVcred(feeEth, ethToVcredRate);

    // 0.01 ETH * 2000 VCRED/ETH = 20 VCRED
    expect(feeVcred).toBe(20n * 10n ** 6n);
  });
  ...
  it('handles different exchange rates', () => {
    ...
    // Low rate
    const lowRate = 100n * 10n ** 6n; // 1 ETH = 100 VCRED
    expect(convertFeeToVcred(feeEth, lowRate)).toBe(10n * 10n ** 6n);

    // High rate
    const highRate = 10000n * 10n ** 6n; // 1 ETH = 10000 VCRED
    expect(convertFeeToVcred(feeEth, highRate)).toBe(1000n * 10n ** 6n);
  });
});
